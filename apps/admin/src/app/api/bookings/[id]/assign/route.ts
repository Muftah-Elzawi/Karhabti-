import { assignBookingSchema } from '@karhabti/validation';
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  return withParsedBody(request, assignBookingSchema, async (input) => {
    const booking = await apiFetchAuthed<BookingDto>(
      `/admin/bookings/${encodeURIComponent(params.id)}/assign`,
      {
        method: 'POST',
