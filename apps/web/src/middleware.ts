...
/**
 * Protects authenticated pages. When the short-lived access cookie is gone
 * but a refresh cookie exists, rotates the pair transparently so the page
 * render (and the user) never notices the expiry.
 */
...
export const config = {
  matcher: ['/profile', '/bookings/:path*', '/book/:path*', '/provider/:path*'],
};
